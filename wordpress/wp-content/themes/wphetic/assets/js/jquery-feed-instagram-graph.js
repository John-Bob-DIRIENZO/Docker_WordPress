...
if (jQuery('#instagram-feed1').length != 0) {
    var token = document.currentScript.getAttribute('token');
    var fields =
        'id,media_type,media_url,thumbnail_url,timestamp,permalink,caption';
    var limit = document.currentScript.getAttribute('nbre_posts'); // Set a number of display items
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            for (var x in response.data) {
                ...
                jQuery('#instagram-feed1').append(html);
            }
        },
        error: function (data) {
